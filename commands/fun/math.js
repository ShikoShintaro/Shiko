...
module.exports = {
    ...
    run: async (client, message, args) => {
        if (cd.has(message.author.id)) return message.channel.send(new MessageEmbed()
            .setTitle('OI')
        ...
        if (isNaN(level)) return message.channel.send(new MessageEmbed()
            .setTitle('OI')
        ...
        if (level <= 0 || level > 20) return message.channel.send(new MessageEmbed()
            .setTitle("OI")
        ...
        if (isNaN(operator_amount)) return message.channel.send(new MessageEmbed()
            .setTitle('Aweee~')
        ...
        if (!equation) return message.channel.send(new MessageEmbed()
            .setTitle('A-')
